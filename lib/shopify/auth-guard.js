...
import { AuthError, ForbiddenError } from '@/lib/util/errors';
...
export async function requireShop(request) {
  ...
  if (!shop) throw new AuthError('This store is not installed.');
  if (!shop.isActive) throw new ForbiddenError('CreditLoop is no longer installed on this store.');
