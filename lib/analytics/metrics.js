...
export async function getOverviewMetrics({ shopId, currencyCode, range = '30d', from, to }) {
  ...
  // Everything the overview needs, issued at once. Nothing here depends on
  // anything else, so running them in sequence only added latency.
  const [current, previous, outstanding, creditOrders, prevCreditOrders, repeat] =
    await Promise.all([
      ...
      getRepeatPurchaseStats({ shopId, currencyCode, start, end }),
    ]);
...
export async function creditTotals({ shopId, currencyCode, start, end }) {
  // Issued and bonus share a filter, so they are one aggregate rather than two
  // round trips — each round trip is a network hop from the serverless
  // function to the database, and the overview makes a lot of them.
  ...
  return {
    ...
    bonus: round2(credits._sum.bonusAmount || 0),
  };
}
