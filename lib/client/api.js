...
export async function apiFetch(path, { method = 'GET', body, signal, _retried = false } = {}) {
  ...
  // A write can change anything the merchant is looking at, so cached reads are
  // dropped rather than left to age out and show a stale figure.
  if (response.ok && method !== 'GET') invalidateApiCache('/api/');

  if (!response.ok || json?.ok === false) {
    const code = json?.error?.code;

    // Session tokens are short-lived, and a stored access token can be
    // invalidated by a scope change. Either way the server has already cleared
    // what was stale, so one retry succeeds without troubling the merchant.
    if ((code === 'SESSION_TOKEN_EXPIRED' || code === 'INVALID_ACCESS_TOKEN') && !_retried) {
