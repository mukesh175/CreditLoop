...
import { invalidateSession } from './sessions';
...
export async function adminGraphql(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt += 1) {
    ...
    if (response.status === 401) {
      // The stored token is no longer valid — typically because the app's
      // scopes changed. Drop it so the next request exchanges the session token
      // for a fresh one, rather than telling the merchant to reinstall.
    ...
    if (response.status === 403) {
      ...
      // token would not help, so the token is left alone.
